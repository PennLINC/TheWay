...
CSV_DIR=csvs
mkdir ${CSV_DIR}
output_file=${CSV_DIR}/${subid}_qsiprep_audit.csv
datalad get -n inputs/data
INPUT_ZIP=$(ls inputs/data/${subid}_qsiprep*.zip | cut -d '@' -f 1 || true)
if [ ! -z "${INPUT_ZIP}" ]; then
...
datalad run -i code/concatenator.py -i csvs/ -o ${PROJECT_ROOT}/concat_ds/group_report.csv --expand inputs --explicit "python code/concatenator.py ${PROJECT_ROOT}/concat_ds/csvs ${PROJECT_ROOT}/QSIPREP_AUDIT.csv"
