...
FREESURFER_ZIP=$(ls inputs/data/freesurfer/${subid}_${sesid}_free*.zip | cut -d '@' -f 1 || true)
...
datalad run \
    ...
    -i ${FREESURFER_ZIP} \
    --explicit \
    -o ${subid}_${sesid}_fmriprep-22.0.2.zip \
    --expand outputs \
    -m "fmriprep:22.0.2 ${subid} ${sesid}" \
...
cat > code/fmriprep-anat_zip.sh << "EOT"
#!/bin/bash
set -e -u -x
subid="$1"
sesid="$2"
...
cd prep
7z a ../${subid}_${sesid}_fmriprep-22.0.2.zip fmriprep
rm -rf prep .git/tmp/wkdir
EOT

chmod +x code/fmriprep-anat_zip.sh
cp ${FREESURFER_HOME}/license.txt code/license.txt
