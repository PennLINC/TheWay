...
then
    echo "Required argument is the path to the qsirecon bootstrap directory."
...
ZIPS=$(find inputs/data -name 'sub-*qsirecon*' | cut -d '/' -f 3 | sort)
if [ -z "${ZIPS}" ]
then
...
datalad run \
    -i code/get_files.sh \
    -i inputs/data/${subid}_qsirecon*.zip \
    --explicit \
    -o ${subid} \
    -m "unzipped ${subid}" \
    "bash code/get_files.sh inputs/data/${subid}_qsirecon*.zip"
...
# unzip outputs
unzip -n $ZIP_FILE 'qsirecon/*' -d .
# copy outputs out of qsirecon
cp -r qsirecon/* .
# remove unzip dir
rm -rf qsirecon
EOT
